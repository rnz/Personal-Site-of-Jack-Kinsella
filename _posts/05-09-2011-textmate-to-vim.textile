...
subtitle: Where I explain how to reproduce over 100 commands from Textmate in VIM.
/* published: false*/
author: Jack Kinsella

---

When I switched from Textmate to VIM I felt like a tourist lost on a Japanese subway without a guide, translator or hope. I was unable to find in VIM the features I'd come to love and rely on in Textmate. I yearned for ⌘ + T to summon my models, and I longer for a project drawer to manage my app's files. Instead of accommodating my needs, VIM sat firm and unresponsive and spat error after error at my clumsy fingers. Even though I was supposed to go cold turkey on Textmate, I met her by night in the yard for a guilty after-hours edit.

With time I learned to mimic nearly every Textmate feature in VIM, and then some. This guide isn't about the powerful features VIM has over-and-above Textmate; you'll figure these out with time. This guide is to help you do in VIM what you already can using Textmate, only with considerably less pain than I underwent.

This guide makes a few preliminary assumptions:
# You've installed Mac VIM: 
@brew install macvim@
# You've installed "Janus":https://github.com/carlhuda/janus, the community maintained VIM distro with plugins and configurations useful to Rails programmers used to Textmate.
# You've created a ~/.vimrc.local file (for core VIM configurations), ~/.gvimrc.local (for configuring the graphics in VIM) and ~/.janus.rake (for adding or removing plugins VIM). See my "dotfiles":https://github.com/jackkinsella/dotfiles/tree/ for some commented examples. 
# You know the basics of VIM. Type vimtutor in your terminal for a 20 minute tutorial. Advanced training can be found be referring to the "vim training resources below":#vim_training_resources .
# You were a power user of the Textmate features.
# (Not strictly necessary) You are a Rails programmer switching from Textmate to VIM. About 30% of this guide is Rails specific.
...
* In general the keyboard shortcuts in Textmate require you to press the keys at the same time. Below, this is symbolised by a +. VIM's shortcuts, by contrast, require you to press the keys in quick succession, rather than at the same time. 
* Whilst commands in Textmate are _case insensitive_, or rather the shift key is specified in the commands below, the commands in VIM are _case sensitive_. If you are told to type Y then typing y will not suffice. 
...
* If you instructed to add something to your vimrc.local or gvimrc.local files then you will need to restart VIM for these changes to take effect. 
* If you are instructed to add code to your .janus.rake file you must cd into your .vim directory and run rake for these changes will be installed. 
* Regular expressions in VIM differ from those in Textmate and this is outside the scope of this guide. Consult the "resources below":#vim_training_resources for an excellent tutorial.
...
_You will be in a new file. Type something and then save with_
 :w newfilename |
|Quit| ⌘ + q | :q 
...
:0,$!sort (0,$ is a range representing whole file) |

h2. Searching Files

|Search file upwards for TOM | ⇧ + ⌃ + S TOM | ?TOM 
_Then press enter once then repeatedly press n or N to continue searching up or down_|
...
_Where 'a' is a bookmark previous set. Type :marks to see all marks._|

h2. Uppercase/Lowercase

|Upcase selected text|⌃ + ⇧ + U| ~ |
|Downcase selected text|⌃ + ⇧ + U| ~ |
|Titlecase current text| ⌃ + OPTION + U | No exact equivalent 
...
|Autocomplete filename| No equivalent | ⌃ + X ⌃ + F 
...
h2. Autoclosing quotations, html tags, brackets and so forth.
...
|Close current html tag|⌘ + ⌥ + .| No exact equivalent. Instead you have auto tag creation. To create <p></p> type p then  ⌃ + X Spacebar 
...
|Snippets (e.g. validates present of)| [when in insert mode] vp<Tab> | vp ⌃ + ] 
