...
When I switched from Textmate to VIM I felt like a tourist lost on a Japanese subway without a guide, translator or hope. I was unable to find in VIM the features I'd come to love and rely on in Textmate. I yearned for ⌘ + T to summon my models and for the Project Drawer to easily overview my app. Instead of accommodating my needs, VIM sat firm and unresponsive spitting error after error in my face. Even though I was supposedly going cold turkey on Textmate, I met her by night in the yard for a guilty after-hours edit.

With time I learned to mimic nearly every Textmate feature in VIM, and then some. This guide isn't about the powerful features VIM has over-and-above Textmate; you'll figure these out on your own soon enough. No, this guide is to help you achieve in VIM what you already can using Textmate, only with considerably less pain than I underwent.

This guide makes a few preliminary assumptions:
# You've installed Mac VIM: 
@brew install macvim@
# You've installed "Janus":https://github.com/carlhuda/janus, the community maintained VIM distro with plugins and configurations useful to web programmers.
...
In general the keyboard shortcuts in Textmate require you to press the keys at the same time. Below, this is symbolised by a +. VIM's shortcuts, by contrast, require you to press the keys in quick succession, rather than at the same time. Whilst commands in Textmate are case insensitive or rather, the shift key is specified in the commands below, the commands in VIM are case sensitive. If you are told to type Y then typing y will not suffice. If you instructed to add something to your vimrc.local or gvimrc.local files then you will need to restart VIM for these changes to take effect. If you are instructed to add code to your .janus.rake file you must cd into your .vim directory and run rake for these changes will be installed. Finally regular expressions in VIM differ from those in Textmate. Consult the "resources below":#vim_training_resources for an appropriate guide.
...
_Erases character under caret |
|Delete selected text| Backspace | d |

h2. Moving Text Around

|Move selected text up|⌃ + ⌘ + ↑ (or ↓)| :m 22 (moves current line to line 22. Easier if line numbers are on with :set number ). alternatively :m+ or :m+N or :m-N for moving 1 or N lines up or down|
