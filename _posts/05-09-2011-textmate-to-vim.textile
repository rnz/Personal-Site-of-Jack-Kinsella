...
|Change size of project drawer| Use mouse to adjust size | let NERDTreeWinSize=16 [vimrc] |
...
|Drop tab| ⌃ + w | ⌃ + w |
|Switch tabs left | ⇧ + ⌘ + [ (left) ] | :tabp |
|Switch tabs right | ⇧ + ⌘ + right | :tabn |
|Go to tab| ⌘[1-9] | ⌘[1-9]|
...
|Move down one line (with caret at same position)| CNRL + N| j|
|Go to line 20 | ⌘ + L 20 | :20 |
...
|Create a posts controller | none |:Rco posts! [note the exclamation mark]|
...
|Snippets (e.g. validates present of)| vp<Tab> | Currently clashes with Command-T since both use T. I've filed numerous but reports. If anyone has fixed this let me know. In many cases, however, typing the TM abbreviation follwed by ]⌃ works fine|
